...
run_deseq <- function(count_dataframe, coldata, count_filter, condition_name) {
  # filter genes based on the count_filter
  filtered_counts <- count_dataframe[rowSums(count_dataframe) >= count_filter,]
  
  # convert filtered_counts dataframe into matrix
  count_mat <- as.matrix(filtered_counts)
  
  # set design formula to condition for normalizaion
  ...
  # get DESeq results
  deseq_results <- results(dds, name=condition_name)
  
  # set rownames to deseq results
  rownames(deseq_results) <- rownames(filtered_counts)
...
run_edger <- function(count_dataframe, group) {
  # dge is a DGE object and has a grouping factor added to it
  ...
  # both common dispersion and tagwise dispersion approaches
  et <- exactTest(dge)
  
  # extract the relevant information from et
  logFC <- et$table$logFC
  logCPM <- et$table$logCPM
  PValue <- et$table$PValue
  
  # create a data frame
  edger_res <- data.frame(logFC, logCPM, PValue)
...
run_limma <- function(counts_dataframe, design, group) {
  ...
  fit <- lmFit(v, design)
  fit <- eBayes(fit)
  result <- topTable(fit, coef = 1, number = 15026, sort.by = "P")
  return(result)
}
...
combine_pval <- function(deseq, edger, limma) {
  combined_pvals <- rbind(
    data.frame(package = "deseq", pval = deseq$pvalue),
    data.frame(package = "edger", pval = edger$PValue),
    data.frame(package = "limma", pval = limma$P.Value)
  )
  return(combined_pvals)
}

#' Create three separate facets for each of the diff. exp. packages.
#'
#' @param deseq Results from DESeq2
#' @param edger Results from edgeR
#' @param limma Results from Limma
...
create_facets <- function(deseq, edger, limma) {
  # Combine results from different packages into a single dataframe
  deseq_df <- data.frame(logFC = deseq$log2FoldChange, padj = deseq$padj, package = "DESeq2")
  edger_df <- data.frame(logFC = edger$logFC, padj = edger$padj, package = "edgeR")
  limma_df <- data.frame(logFC = limma$logFC, padj = limma$adj.P.Val, package = "Limma")
  
  # Combine the dataframes into one
  combined_data <- rbind(deseq_df, edger_df, limma_df)
...
theme_plot <- function(volcano_data) {
  ...
  p <- ggplot(volcano_data, aes(x = logFC, y = -log10(padj), color = as.factor(padj < 1e-100))) +
